...
#include "mxfp6_gemm/kernel_swap_m32.hpp"
#include "mxfp6_gemm/kernel_large.hpp"
...
void configure_scheduler(Arguments& arguments,
  ...
  arguments.scheduler.raster_order = raster_order == RasterOrder::AlongN
      ? SchedulerRaster::AlongN
      : SchedulerRaster::AlongM;
...
at::Tensor gemm_cuda_impl(at::Tensor const& a,
...
#else
  TORCH_CHECK((m >= 1 && m <= 32) || m == 2048,
              "this benchmark-tuned kernel requires 1 <= m <= 32 or m == 2048; got ", m);
  TORCH_CHECK(n > 0 && n % 128 == 0,
              "n must be a positive multiple of 128; got ", n);
  ...
  int64_t const a_bytes = m * k * 3 / 4;
  int64_t const b_bytes = n * k * 3 / 4;
  ...
  // These are the CUTLASS profiler winners for the five requested bs2048
  // problems. Fixed-shape exhaustive mode selected the mainloop schedule,
  // tile, raster order, and swizzle independently for every shape.
  if (m == 2048) {
    if (n == 3072 && k == 5120) {
      return launch_normal<mxfp6_gemm::large_m::Kernel64x128x128Pingpong>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
          1, RasterOrder::AlongN);
    }
    if (n == 5120 && (k == 7168 || k == 17408)) {
      return launch_normal<mxfp6_gemm::large_m::Kernel128x128x128Cooperative>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
          1, RasterOrder::AlongN);
    }
    if (n == 5120 && k == 8192) {
      return launch_normal<mxfp6_gemm::large_m::Kernel128x128x128Pingpong>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
          2, RasterOrder::AlongM);
    }
    if (n == 8704 && k == 5120) {
      return launch_normal<mxfp6_gemm::large_m::Kernel128x128x128StreamK>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
  ...
  // Swap A/B: D.T [N,M] = B [N,K] @ A.T [K,M]. A column-major [N,M]
  // output aliases the requested row-major D [M,N] byte-for-byte. Profiler
  // exhaustive profiler ranking supplies the candidate set. A second ranking
  // under the reference benchmark's 8 GB L2 flush selects Stream-K for the
  // first four N/K pairs and the non-Stream-K cooperative path for 8704x5120.
  if (m == 1) {
    return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N8StreamK>(
        a, b, sfa, sfb, m, n, k, alpha, device_index,
        1, RasterOrder::AlongM, 2);
  }
  if (m == 16) {
    if (n == 8704 && k == 5120) {
      return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N8Cooperative>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
  ...
  if (m == 32) {
    if (n == 8704 && k == 5120) {
      return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N16Cooperative>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
          1, RasterOrder::AlongM);
    }
    return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N16StreamK>(
        a, b, sfa, sfb, m, n, k, alpha, device_index,
        1, RasterOrder::AlongM, 2);
  }
  if (m <= 16) {
    if (n == 5120 && k == 17408) {
      if (m > 1) {
        return launch_swapped<
            mxfp6_gemm::swap_m32::KernelM64N16K512Pingpong>(
            a, b, sfa, sfb, m, n, k, alpha, device_index);
      }
      auto raster = m == 1 ? RasterOrder::AlongM : RasterOrder::AlongN;
      int swizzle = 4;
      return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N8StreamK>(
          a, b, sfa, sfb, m, n, k, alpha, device_index,
          swizzle, raster, 2);
    ...
    if (n == 3072 && k == 5120) {
      return launch_swapped<mxfp6_gemm::swap_m32::KernelM64N16K256Pingpong>(
          a, b, sfa, sfb, m, n, k, alpha, device_index);
    }
    if (m == 1 && n == 5120 && k == 8192) {
      return launch_swapped<mxfp6_gemm::swap_m32::KernelM64N16K512Pingpong>(
          a, b, sfa, sfb, m, n, k, alpha, device_index);
    }
    if (m == 1 && n == 5120 && k == 7168) {
      return launch_swapped<mxfp6_gemm::swap_m32::KernelM64N16Pingpong>(
          a, b, sfa, sfb, m, n, k, alpha, device_index);
    }
    int swizzle = 1;
    auto raster = RasterOrder::AlongM;
    if (m > 1 && n == 5120 && k == 7168) {
      raster = RasterOrder::AlongN;
    }
    return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N8Cooperative>(
        a, b, sfa, sfb, m, n, k, alpha, device_index, swizzle, raster);
  }
  if (n == 5120 && k == 17408) {
    return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N16StreamK>(
        a, b, sfa, sfb, m, n, k, alpha, device_index,
        8, RasterOrder::AlongM, 2);
  }
  if (n == 3072 && k == 5120) {
    return launch_swapped<mxfp6_gemm::swap_m32::KernelM64N16K256Pingpong>(
        a, b, sfa, sfb, m, n, k, alpha, device_index);
  }
  if (n == 5120 && (k == 7168 || k == 8192)) {
    return launch_swapped<mxfp6_gemm::swap_m32::KernelM64N16Pingpong>(
        a, b, sfa, sfb, m, n, k, alpha, device_index);
  }
  return launch_swapped<mxfp6_gemm::swap_m32::KernelM128N16Cooperative>(
